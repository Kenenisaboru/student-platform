...
import { Users, TrendingUp, Sparkles, BookOpen, ExternalLink } from 'lucide-react';
...
const RightSidebar = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        // Show up to 5 recent users as "active" users
  ...
  const trendingTags = [
    { tag: 'Engineering', posts: 28 },
    { tag: 'Medicine', posts: 22 },
    { tag: 'StudyTips', posts: 19 },
    { tag: 'Campus', posts: 15 },
    { tag: 'JobBoard', posts: 12 },
  ];
  ...
  return (
    <aside className="hidden xl:flex flex-col w-[280px] shrink-0 sticky top-24 h-[calc(100vh-7rem)] overflow-y-auto pl-2 pb-8 space-y-5 scrollbar-thin" id="right-sidebar">
      ...
      <div className="p-5 rounded-2xl bg-white/[0.03] border border-white/[0.05] backdrop-blur-sm">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="p-3 rounded-xl bg-white/[0.03] border border-white/[0.04]">
            <p className="text-2xl font-extrabold text-white">{topUsers.length}+</p>
      ...
      <div className="p-5 rounded-2xl bg-gradient-to-br from-indigo-600/5 to-purple-600/5 border border-indigo-500/10">
        ...
        <button className="w-full text-[12px] font-bold text-indigo-400 hover:text-indigo-300 bg-indigo-500/10 hover:bg-indigo-500/15 py-2.5 rounded-xl transition-all flex items-center justify-center gap-1.5">
