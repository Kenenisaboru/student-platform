...
const Navbar = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 flex flex-col">
      {/* Top Banner */}
      <div className="bg-slate-900 text-white text-[10px] sm:text-xs font-semibold py-1.5 px-4 hidden sm:flex justify-between items-center z-50">
      ...
      <nav className="bg-white/80 backdrop-blur-xl border-b border-slate-200/50 shadow-sm transition-all duration-300">
        <div className="container mx-auto px-4 max-w-6xl h-16 sm:h-20 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-1 sm:space-x-3">
            {user ? (
              <>
                <Link to="/" title="Home" className={`p-2.5 rounded-xl transition-all duration-300 ${isActive('/') ? 'text-primary-600 bg-primary-50' : 'text-slate-500 hover:text-primary-600 hover:bg-primary-50'}`}>
                  <Home className="w-5 h-5 sm:w-5 sm:h-5" />
                </Link>
                <Link to="/notifications" title="Notifications" className={`p-2.5 rounded-xl transition-all duration-300 relative group ${isActive('/notifications') ? 'text-primary-600 bg-primary-50' : 'text-slate-500 hover:text-primary-600 hover:bg-primary-50'}`}>
                  <Bell className={`w-5 h-5 sm:w-5 sm:h-5 ${!isActive('/notifications') && 'group-hover:animate-[wiggle_1s_ease-in-out_infinite]'}`} />
                  <span className="absolute top-2 right-2.5 w-2 h-2 bg-rose-500 border-2 border-white rounded-full"></span>
                </Link>
                <Link to="/create-post" title="Create Post" className={`p-2.5 rounded-xl transition-all duration-300 ${isActive('/create-post') ? 'text-primary-600 bg-primary-50' : 'text-slate-500 hover:text-primary-600 hover:bg-primary-50'}`}>
                  <PlusSquare className="w-5 h-5 sm:w-5 sm:h-5" />
                </Link>

                {/* Profile Dropdown Trigger */}
                <Link to={`/profile/${user?._id}`} title="Profile" className={`ml-2 pl-2 border-l border-slate-200 hidden sm:flex items-center space-x-2 transition-opacity ${isActive(`/profile/${user?._id}`) ? 'opacity-100' : 'hover:opacity-80'}`}>
                  <img 
                    src={user?.profilePicture} 
                    alt={user?.name} 
                    className={`w-9 h-9 rounded-full object-cover shadow-sm border-2 ring-2 transition-all ${isActive(`/profile/${user?._id}`) ? 'border-primary-100 ring-primary-50' : 'border-white ring-slate-100'}`} 
                  />
                  <div className="flex flex-col">
                    <span className={`text-sm font-bold leading-none ${isActive(`/profile/${user?._id}`) ? 'text-primary-600' : 'text-slate-700'}`}>{user?.name?.split(' ')[0]}</span>
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{user?.role}</span>
                  </div>
                </Link>

                {user?.role === 'admin' && (
                  <Link to="/admin" title="Admin Panel" className={`p-2.5 rounded-xl transition-all duration-300 hidden sm:flex border ${isActive('/admin') ? 'text-indigo-600 bg-indigo-50 border-indigo-200' : 'text-indigo-500 hover:text-indigo-600 hover:bg-indigo-50 border-indigo-100/50'}`}>
