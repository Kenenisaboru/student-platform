...
const Profile = () => {
  ...
  if (!user) return (
    <div className="max-w-2xl mx-auto py-24 px-4 text-center">
      ...
      <button onClick={() => window.history.back()} className="px-8 py-3 bg-white/5 border border-white/10 text-white rounded-xl font-bold transition-all">Previous Terminal</button>
    </div>
  );
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-4xl mx-auto py-6 px-4 pb-24">
      ...
      {/* Pro Profile Header */}
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="bg-[#0a0f1e]/80 backdrop-blur-3xl rounded-[2.5rem] overflow-hidden border border-white/8 shadow-2xl relative mb-10">
        
        {/* Dynamic Header Banner */}
        <div className="h-44 sm:h-64 bg-linear-to-br from-blue-600 via-indigo-900 to-black relative overflow-hidden">
           <div className="absolute inset-0 bg-grid-white/[0.05] mask-[linear-gradient(0deg,white,rgba(255,255,255,0.5))]"></div>
           <div className="absolute top-0 right-0 w-100 h-100 bg-blue-500/20 rounded-full blur-[120px] -mr-40 -mt-40"></div>
           <div className="absolute bottom-0 left-0 w-75 h-75 bg-indigo-600/20 rounded-full blur-[100px] -ml-20 -mb-20"></div>
        ...
        <div className="px-5 sm:px-8 pb-10 relative">
          <div className="flex flex-col md:flex-row md:items-end -mt-16 sm:-mt-24 mb-10 gap-6 sm:gap-8">
            <motion.div 
            ...
            >
              <div className="relative p-1.5 rounded-[2.5rem] bg-linear-to-br from-white/20 to-white/5 backdrop-blur-3xl shadow-2xl">
                <div className="relative overflow-hidden rounded-3xl border-4 border-[#060a14]">
                    <img src={editing ? editData.profilePicture : user.profilePicture} className={`w-28 h-28 sm:w-44 sm:h-44 object-cover transition-all duration-500 ${uploading ? 'opacity-50 blur-sm scale-110' : 'group-hover:scale-105'}`} alt={user.name} />
                    {editing && (
              ...
              <div className="absolute -bottom-1 -right-1 w-9 h-9 sm:w-10 sm:h-10 bg-blue-600 rounded-2xl flex items-center justify-center border-4 border-[#060a14] shadow-xl text-white">
            ...
            <div className="flex-1 pb-1">
              <div className="flex items-center gap-3 mb-2 flex-wrap">
                 <h1 className="text-2xl sm:text-4xl md:text-5xl font-black text-white tracking-tighter leading-none">{user.name}</h1>
              ...
              <div className="flex items-center text-slate-500 font-black mb-6 space-x-2 text-xs uppercase tracking-widest flex-wrap gap-y-1">
              ...
              <div className="grid grid-cols-3 gap-3 sm:gap-6 max-w-sm">
                {[
                  ...
                  <div key={stat.label} className="flex flex-col">
                    <div className="flex items-center gap-1.5 text-white">
                       <span className="text-lg sm:text-xl font-black tracking-tight tabular-nums">{stat.val}</span>
                       <stat.icon className={`w-3.5 h-3.5 ${stat.color}`} />
                    </div>
                    <span className="text-[9px] sm:text-[10px] font-black text-slate-600 uppercase tracking-widest mt-0.5">{stat.label}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Action Terminal */}
            <div className="flex flex-col gap-3 w-full md:w-auto md:min-w-45">
              {isOwnProfile ? (
                ...
                <div className="flex gap-2 w-full">
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={handleFollow} disabled={followLoading}
                    className={`flex-1 py-3.5 rounded-2xl flex items-center justify-center font-black transition-all text-xs uppercase tracking-widest border ${
                      isFollowing
                        ? 'bg-white/4 text-white border-white/10 hover:bg-rose-500/10 hover:text-rose-400 hover:border-rose-500/30'
                        : 'bg-linear-to-r from-blue-600 to-indigo-600 text-white border-white/10 shadow-lg shadow-blue-600/20'
                    }`}>
                  ...
                  <Link to={`/messages/${id}`}
                    className="aspect-square bg-white/4 hover:bg-white/8 text-white p-3.5 rounded-2xl transition-all border border-white/10 flex items-center justify-center backdrop-blur-xl">
            ...
            {!editing ? (
              <motion.div key="view" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-8">
                <div className="relative">
                  ...
                  <p className="text-slate-400 text-sm sm:text-[15px] leading-[1.8] font-medium bg-white/2 p-6 sm:p-8 rounded-3xl border border-white/6 italic relative overflow-hidden group shadow-inner">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                  {[
                    ...
                    <div key={card.label} className={`flex flex-col p-6 rounded-3xl bg-linear-to-br ${card.bg} border border-white/6 hover:border-white/10 transition-all group`}>
                      <div className={`w-10 h-10 rounded-xl bg-white/3 flex items-center justify-center ${card.color} mb-4 group-hover:scale-110 transition-all border border-white/5`}>
              ...
              <motion.form key="edit" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} onSubmit={handleUpdate} className="space-y-8 bg-white/3 p-8 sm:p-10 rounded-[2.5rem] border border-white/8 backdrop-blur-xl shadow-inner relative">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-2">
                    ...
                    <div className="relative group/input">
                       <input type="text" className="w-full bg-white/4 border border-white/10 rounded-2xl py-4 pl-12 pr-4 focus:border-blue-500/50 outline-none transition-all font-black text-white text-sm tracking-tight shadow-inner" value={editData.name} onChange={(e) => setEditData({...editData, name: e.target.value})} />
                  ...
                  <div className="space-y-2">
                    ...
                    <label className={`flex items-center justify-between px-6 py-4 bg-white/4 border border-white/10 rounded-2xl cursor-pointer hover:border-blue-500/40 hover:bg-white/8 transition-all font-black text-slate-400 text-sm tracking-tight ${uploading ? 'opacity-30' : ''}`}>
      ...
      {isOwnProfile && (
        <div className="max-w-xl mx-auto mb-10 flex gap-2 p-1.5 bg-[#0d1428] border border-white/8 rounded-3xl shadow-2xl relative overflow-hidden">
          <div className="absolute inset-0 bg-blue-600/5 blur-xl"></div>
          <button 
            onClick={() => setActiveTab('posts')}
            className={`relative z-10 flex-1 py-3.5 rounded-3xl font-black text-xs uppercase tracking-widest transition-all ${activeTab === 'posts' ? 'bg-white text-[#060a14] shadow-xl' : 'text-slate-500 hover:text-white hover:bg-white/3'}`}
          >
            My Activity
          </button>
          <button 
            onClick={() => setActiveTab('saved')}
            className={`relative z-10 flex-1 py-3.5 rounded-3xl font-black text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-3 ${activeTab === 'saved' ? 'bg-white text-[#060a14] shadow-xl' : 'text-slate-500 hover:text-white hover:bg-white/3'}`}
          >
